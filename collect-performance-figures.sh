...
OUTPUT="$(./find-communities-OpenMP $INPUT_FILE -f 2 -b $BENCHMARK_RUNS -a 1 -t 1 | grep -E 'Algorithm version|Number of threads:                |Average execution time|Minimum execution time' )"
echo $OUTPUT >> $OUTPUT_FILE
...
do
	...
	OUTPUT="$(./find-communities-OpenMP $INPUT_FILE -f $INPUT_FORMAT -b $BENCHMARK_RUNS -a 1 -t $i | grep -E 'Algorithm version|Number of threads:                |Average execution time|Minimum execution time' )"
	echo $OUTPUT >> $OUTPUT_FILE
