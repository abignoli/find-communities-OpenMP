...
OUTPUT="$(./find-communities-OpenMP $INPUT_FILE -f $INPUT_FORMAT -b $BENCHMARK_RUNS -a 0 $OTHER_OPTIONS | grep -E 'Algorithm version|Number of threads:                |Average execution time|Minimum execution time' )"
echo $OUTPUT > $OUTPUT_FILE
SEQUENTIAL_TIME_AVERAGE="$(echo $OUTPUT | awk '{print $7}')"
SEQUENTIAL_TIME_AVERAGE=${SEQUENTIAL_TIME_AVERAGE::-1}
SEQUENTIAL_TIME_MINIMUM="$(echo $OUTPUT | awk '{print $11}')"
SEQUENTIAL_TIME_MINIMUM=${SEQUENTIAL_TIME_AVERAGE::-1}

printf "\n\n"

echo "Running parallel (Sort & Merge) version with 1 thread"
echo "Running parallel (Sort & Merge) version with 1 thread" >> $OUTPUT_FILE
OUTPUT="$(./find-communities-OpenMP $INPUT_FILE -f 2 -b $BENCHMARK_RUNS -a 1 -t 1 $OTHER_OPTIONS | grep -E 'Algorithm version|Number of threads:                |Average execution time|Minimum execution time' )"
echo $OUTPUT >> $OUTPUT_FILE
...
do
	...
	OUTPUT="$(./find-communities-OpenMP $INPUT_FILE -f $INPUT_FORMAT -b $BENCHMARK_RUNS -a 1 -t $i $OTHER_OPTIONS | grep -E 'Algorithm version|Number of threads:                |Average execution time|Minimum execution time' )"
	echo $OUTPUT >> $OUTPUT_FILE
...
echo "Running parallel (Sort & Merge) version with 1 thread (-e 0)"
echo "Running parallel (Sort & Merge) version with 1 thread (-e 0)" >> $OUTPUT_FILE
OUTPUT="$(./find-communities-OpenMP $INPUT_FILE -f 2 -b $BENCHMARK_RUNS -a 1 -t 1 $OTHER_OPTIONS | grep -E 'Algorithm version|Number of threads:                |Average execution time|Minimum execution time' )"
echo $OUTPUT >> $OUTPUT_FILE
SINGLE_THREAD_TIME_AVERAGE="$(echo $OUTPUT | awk '{print $14}')"
SINGLE_THREAD_TIME_AVERAGE=${SINGLE_THREAD_TIME_AVERAGE::-1}
...
for i in $(seq 2 $MAX_THREADS);
do
	echo "Running parallel (Sort & Merge) version with $i threads (-e 0)"
	echo "Running parallel (Sort & Merge) version with $i threads (-e 0)" >> $OUTPUT_FILE
	OUTPUT="$(./find-communities-OpenMP $INPUT_FILE -f $INPUT_FORMAT -b $BENCHMARK_RUNS -a 1 -t $i $OTHER_OPTIONS | grep -E 'Algorithm version|Number of threads:                |Average execution time|Minimum execution time' )"
	echo $OUTPUT >> $OUTPUT_FILE
	THREADS_TIME="$(echo $OUTPUT | awk '{print $14}')"
	THREADS_TIME=${THREADS_TIME::-1}
...
OUTPUT="$(./find-communities-OpenMP $INPUT_FILE -f 2 -b $BENCHMARK_RUNS -a 2 -t 1 $OTHER_OPTIONS | grep -E 'Algorithm version|Number of threads:                |Average execution time|Minimum execution time' )"
echo $OUTPUT >> $OUTPUT_FILE
...
do
	...
	OUTPUT="$(./find-communities-OpenMP $INPUT_FILE -f $INPUT_FORMAT -b $BENCHMARK_RUNS -a 2 -t $i $OTHER_OPTIONS | grep -E 'Algorithm version|Number of threads:                |Average execution time|Minimum execution time' )"
	echo $OUTPUT >> $OUTPUT_FILE
